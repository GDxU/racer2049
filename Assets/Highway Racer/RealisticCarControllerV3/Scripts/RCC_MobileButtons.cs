...
public class RCC_MobileButtons : MonoBehaviour {
	...
	public IEnumerator GetVehicles(){
		...
		if (!HR_GamePlayHandler.Instance.player.GetComponent<RCC_CarControllerV3>().useNOS && NOSButton && NOSButton.gameObject.activeSelf)
			NOSButton.gameObject.SetActive (false);
	...
	void Update(){
		...
		for (int i = 0; i < carControllers.Length; i++) {
			...
			if(carControllers[i].canControl){
				...
				carControllers[i].gasInput = Mathf.Clamp01(gasInput + (NOSInput - 1f));
				...
				carControllers [i].highBeamHeadLightsOn = headLightsButton.pressing ? true : false;
