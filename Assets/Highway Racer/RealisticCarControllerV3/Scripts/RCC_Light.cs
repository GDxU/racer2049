...
public class RCC_Light : MonoBehaviour {
	...
	private void Projectors(){
		...
		if (HR_GamePlayHandler.Instance) {
			...
			if (HR_GamePlayHandler.Instance.dayOrNight == HR_GamePlayHandler.DayOrNight.Night)
				projector.material.color = _light.color * (_light.intensity / 1f);
			else
				projector.material.color = _light.color * (_light.intensity / 3f);
	...
	private void TrailRenderers(){

		trailRenderer.enabled = _light.enabled;
		trailRenderer.time = Mathf.Lerp (0f, .1f, _light.intensity);
