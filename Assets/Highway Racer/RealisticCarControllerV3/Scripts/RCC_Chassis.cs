...
public class RCC_Chassis : MonoBehaviour {
	...
	void LegacyChassis (){
		...
		verticalLean = Mathf.Clamp(Mathf.Lerp (verticalLean, mainRigid.angularVelocity.x * chassisVerticalLean, Time.fixedDeltaTime * 3f), -3f, 3f);
		horizontalLean = Mathf.Clamp(Mathf.Lerp (horizontalLean, (transform.InverseTransformDirection(mainRigid.angularVelocity).y * (transform.InverseTransformDirection(mainRigid.velocity).z >= 0 ? 1 : -1)) * chassisHorizontalLean, Time.fixedDeltaTime * 3f), -3f, 3f);
