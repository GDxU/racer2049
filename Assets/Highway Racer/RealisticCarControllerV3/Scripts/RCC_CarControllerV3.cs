...
public class RCC_CarControllerV3 : MonoBehaviour {
	...
	internal List<GameObject> allWheelTransforms = new List<GameObject>();
	...
	internal float _steerInput{get{
			...
			return steerInput + verticalAngle;
	...
	public float chassisVerticalLean = 10f;		// Chassis Vertical Lean Sensitivity.
	public float chassisHorizontalLean = 10f;		// Chassis Horizontal Lean Sensitivity.

	// Lights.
	public RCC_Light[] headLights;
	public RCC_Light[] brakeLights;
	public RCC_Light[] reverseLights;
	public RCC_Light[] indicatorLights;
	...
	[Range(0f, 1f)] public float steerHelperAngularVelStrength = .5f;
	[Range(0f, 1f)] public float tractionHelperStrength = .5f;
	...
	void Awake (){
		...
		allWheelTransforms.Add(FrontLeftWheelTransform.gameObject); allWheelTransforms.Add(FrontRightWheelTransform.gameObject); allWheelTransforms.Add(RearLeftWheelTransform.gameObject); allWheelTransforms.Add(RearRightWheelTransform.gameObject);
		GetComponentInChildren<WheelCollider> ().ConfigureVehicleSubsteps (10f, 5, 5);
		...
		List<RCC_Light> _headLights = new List<RCC_Light> ();
		List<RCC_Light> _brakeLights = new List<RCC_Light> ();
		List<RCC_Light> _reverseLights = new List<RCC_Light> ();
		List<RCC_Light> _indicatorLights = new List<RCC_Light> ();

		for (int i = 0; i < lights.Length; i++) {

			if (lights [i].lightType == RCC_Light.LightType.HeadLight)
				_headLights.Add (lights [i]);
			if (lights [i].lightType == RCC_Light.LightType.BrakeLight)
				_brakeLights.Add (lights [i]);
			if (lights [i].lightType == RCC_Light.LightType.ReverseLight)
				_reverseLights.Add (lights [i]);
			if (lights [i].lightType == RCC_Light.LightType.Indicator)
				_indicatorLights.Add (lights [i]);
		}

		headLights = _headLights.ToArray ();
		brakeLights = _brakeLights.ToArray ();
		reverseLights = _reverseLights.ToArray ();
		indicatorLights = _indicatorLights.ToArray ();
		...
		case RCC_Settings.BehaviorType.HighwayRacer:
			...
			steerHelperAngularVelStrength = Mathf.Clamp (steerHelperAngularVelStrength, .5f, 1f);
...
//			highspeedsteerAngleAtspeed = 80f;
	...
	void SoundsInitialize (){
		...
		hornSound = RCC_CreateAudioSource.NewAudioSource(gameObject, "Horn/Siren Sound AudioSource", 5, 50, 1, PlayerPrefsX.GetBool(transform.name + "SirenAttached", false) ? sirenClip : hornClip, true, false, false);
	...
	void Inputs(){
		...
		if (useHelpersForHighway && roadPooling) {
			...
			if (transform.position.x > roadPooling.roadWidth)
				steerInput = Mathf.Clamp (steerInput, -1f, 0f);

			if (transform.position.x < -roadPooling.roadWidth)
				steerInput = Mathf.Clamp (steerInput, 0f, 1f);
...
//			if (Mathf.Abs (steerInput) > .1f) {
//				if (transform.position.x < roadPooling.roadWidth && transform.position.x > -roadPooling.roadWidth)
//					rigid.velocity = new Vector3 (Mathf.Lerp (rigid.velocity.x, steerInput * 5f, Time.fixedDeltaTime / 5f), rigid.velocity.y, rigid.velocity.z);
//				else if (transform.position.x < roadPooling.roadWidth)
//					rigid.velocity = new Vector3 (Mathf.Lerp (rigid.velocity.x, Mathf.Clamp01 (steerInput) * 5f, Time.fixedDeltaTime / 1f), rigid.velocity.y, rigid.velocity.z);
//				else
//					rigid.velocity = new Vector3 (Mathf.Lerp (rigid.velocity.x, Mathf.Clamp (steerInput, -1f, 0f) * 5f, Time.fixedDeltaTime / 1f), rigid.velocity.y, rigid.velocity.z);
//			} else {
//				if (transform.position.x < roadPooling.roadWidth && transform.position.x > -roadPooling.roadWidth)
//					rigid.velocity = new Vector3 (Mathf.Lerp (rigid.velocity.x, 0f, Time.fixedDeltaTime / .2f), rigid.velocity.y, rigid.velocity.z);
//			}
//
//			if (Mathf.Abs (steerInput) > .1f)
//				rigid.angularVelocity = new Vector3 (rigid.angularVelocity.x, Mathf.Lerp (rigid.angularVelocity.y, (angle) / .5f, Time.deltaTime * (speed / 100f)), rigid.angularVelocity.z);
//			else
//				rigid.angularVelocity = new Vector3 (rigid.angularVelocity.x, Mathf.Lerp (rigid.angularVelocity.y, (angle) / .5f, Time.deltaTime * (speed / 100f)), rigid.angularVelocity.z);
//
//			if (transform.position.x < roadPooling.roadWidth && transform.position.x > -roadPooling.roadWidth)
//				rigid.AddForce (Vector3.right * (steerInput * highwaySteeringHelper), ForceMode.Acceleration);

			if(speed > 25f)
				rigid.AddForce (-Vector3.forward * (brakeInput * highwayBrakingHelper * 2f), ForceMode.Acceleration);

		}
			
		rigid.centerOfMass = transform.InverseTransformPoint(COM.transform.position);
		
		steerHelperLinearVelStrength = Mathf.Lerp(0f, 1f, highwaySteeringHelper / 10f);
	...
	void HRVariables(){
		...
		float angle = Mathf.Clamp (Quaternion.Angle (transform.rotation, Quaternion.identity) / (transform.eulerAngles.y >= 0 && transform.eulerAngles.y <= 180 ? -30f : 30f), -1f, 1f);
		verticalAngle = angle * 2f;
