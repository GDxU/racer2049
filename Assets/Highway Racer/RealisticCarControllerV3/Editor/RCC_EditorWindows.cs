...
public class RCC_EditorWindows : Editor {
	...
	public static void CreateRCCCamera(){
		...
		if (GameObject.FindObjectOfType<HR_CarCamera> ()) {
			...
			Selection.activeGameObject = GameObject.FindObjectOfType<HR_CarCamera> ().gameObject;
	...
	public static void CreateWheelCamera(){
		...
		if(!Selection.activeGameObject.GetComponentInParent<RCC_CarControllerV3>()){
			...
			if(Selection.activeGameObject.GetComponentInParent<RCC_CarControllerV3>().gameObject.GetComponentInChildren<RCC_TPSCamera>()){
				EditorUtility.DisplayDialog("Your Vehicle Has TPS Camera Already!", "Your vehicle has TPS camera already!", "Ok");
				Selection.activeGameObject = Selection.activeGameObject.GetComponentInChildren<RCC_TPSCamera>().gameObject;
				return;
			}

			GameObject wheelCam = new GameObject("TPS Camera");
			wheelCam.transform.SetParent(Selection.activeGameObject.GetComponentInParent<RCC_CarControllerV3>().transform, false);
			wheelCam.AddComponent<RCC_TPSCamera>();
